...
Partial Class kasir_output_list_penerimaan
  Inherits System.Web.UI.Page

  Private Sub kasir_output_list_penerimaan_Load(sender As Object, e As EventArgs) Handles Me.Load
  ...
  Protected Sub TXTTAHUN_SelectedIndexChanged(sender As Object, e As EventArgs)
    ...
    Using cmd As SqlCommand = New SqlCommand(strQuery)
      cmd.Parameters.Add("@TGL_MASUK", SqlDbType.VarChar).Value = Me.TXTTAHUN.Text
      Me.GVBARANG.DataSource = Me.GetData(cmd)
      Me.GVBARANG.DataBind()
      ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Pop", "animatePrint();", True)
    End Using
  ...
  Private Sub GET_SEARCH_SUPPLIER(search As String)
    Dim strQuery As String = "SELECT * FROM [VW_PENERIMAAN] WHERE NAMA_SUPPLIER LIKE @p1 ORDER BY NAMA_SUPPLIER"
  ...
  <WebMethod>
  Public Shared Function GET_DELETE_PENERIMAAN(KODE_PENERIMAAN As String) As Boolean
    Dim conString As String = ConfigurationManager.ConnectionStrings("DB_Penjualan_Butik_ConnectionString").ToString
    Using con As New SqlConnection(conString)
      Using cmd As New SqlCommand("DELETE FROM T_PENERIMAAN WHERE KODE_PENERIMAAN = @KODE_PENERIMAAN")
        cmd.Connection = con
