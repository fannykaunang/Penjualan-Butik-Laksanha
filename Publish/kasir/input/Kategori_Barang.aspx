<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Kategori_Barang.aspx.vb" Inherits="kasir_input_Kategori_Barang" %>
...
<head>
    ...
    <title>Input Kategori Barang | Penjualan Butik</title>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="TXTKDBARANG">Kategori Barang</label>
                                        <asp:TextBox ID="TXTKATEGORIBARANG" runat="server" CssClass="form-control" placeholder="Masukkan Kategori Barang"></asp:TextBox>
                                        <span id="LBINFOKATEGORIBARANG" class="help-block" style="color: #dc3545"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    reserved.
        ...
        <script>
            function getCalculationGrid(button) {
                var btn = $(button);
                var isValid = true;
                if ($('#TXTKATEGORIBARANG').val() == '') {
                    isValid = false;
                    $("[id*=TXTKATEGORIBARANG]").focus();
                    $("[id*=LBINFOKATEGORIBARANG]").text("Kategori Barang jangan dibiarkan kosong!");
                    alertify.error('<img id="Image2" src="../../dist/img/Action_Close_32x32.png" style="height:30px;width:30px;" /><strong> Error!</strong> </br>Kategori Barang jangan dibiarkan kosong!');
                    return false;
                } else {
                    isValid = true;
                    $("[id*=LBINFOKATEGORIBARANG]").text("");
                    $('#Button1').click();
                    return true;
                }
            };
        ...
        <script type="text/javascript">
            $(function () {
                $("#TXTKATEGORIBARANG").change(function () {
                    $("#LBINFOKATEGORIBARANG").text("");
                });
                ...
                $("#TXTKATEGORIBARANG").keyup(function () {
                    var kdbarang = $(this);
                    $.ajax({
                        type: "POST",
                        url: '<%=ResolveUrl("~/kasir/input/Kategori_Barang.aspx/GET_KATEGORIBARANG")%>',
                        data: JSON.stringify({ "KATEGORI": kdbarang.val() }),
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        async: false,
                        success: function (result) {
                            if (result.d.length > 0) {
                                alertify.error('<img id="Image2" src="../../dist/img/Action_Close_32x32.png" style="height:30px;width:30px;" /><strong> Error!</strong> </br>Kategori Barang sudah terdaftar. Tidak dapat menyimpan kategori yang sama');
                                $("[id*=LBINFOKATEGORIBARANG]").text("Kategori Barang sudah terdaftar!");
                                inputfaktur = true;
                            } else {
                                $("[id*=LBINFOKATEGORIBARANG]").text(null);
                                inputfaktur = false;
                            }
                        },
